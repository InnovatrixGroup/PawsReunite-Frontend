  ...
  return (
    <div className='carousel-container relative w-full max-h-96 overflow-hidden'>
      ...
      <div className='carousel__buttons inset-0 absolute flex items-center justify-between px-3'>
          <button onClick={prevSlide} className='carousel__button p-2 rounded-full shadow bg-white/50 text-gray-800 hover:bg-white/80'>
            <KeyboardArrowLeftOutlinedIcon />
          </button>
          <button onClick={nextSlide} className='carousel__button p-2 rounded-full shadow bg-white/50 text-gray-800 hover:bg-white/80'>
            <KeyboardArrowRightOutlinedIcon />
          </button>
      </div>
      <div className='carousel__dots absolute bottom-4 right-0 left-0 flex justify-center items-center gap-2'>
          {images.map((_,index) => ( 
            <div className={`w-1 h-1 bg-gray-400 rounded-full ${curr === index ? 'bg-gray-800 p-1' : ''} `} />
          ))}
      </div>
    </div>
  )
